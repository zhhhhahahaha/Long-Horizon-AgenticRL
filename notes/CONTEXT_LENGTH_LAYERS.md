...
│      来源: run_qwen3p5_4B.sh ROLLOUT_ARGS                               │
...
- **在哪里**: `run_qwen3p5_4B.sh` ROLLOUT_ARGS
...
- **改 L3** (sample 总预算): 改 `run_qwen3p5_4B.sh` 的 `--rollout-max-context-len`. **不用重启 sglang**, 下次 rollout 生效
- **改 L4** (sglang 单次): 改 `run_qwen3p5_4B.sh` 的 `--rollout-max-response-len`. **不用重启 sglang**
